...
# ignored options
$LZHCC -c -O -Wall -g -std=c11 -ffreestanding -fno-builtin \
...
$LZHCC -o /dev/null $tmp/inline1.c $tmp/inline2.c $tmp/inline3.c
check inline

echo 'extern inline void foo() {}' > $tmp/inline1.c
echo 'int foo(); int main() { foo(); }' > $tmp/inline2.c
$LZHCC -o /dev/null $tmp/inline1.c $tmp/inline2.c
check inline
